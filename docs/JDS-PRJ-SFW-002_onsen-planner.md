...
- **Monetization:** StoreKit 2 via `Vecka/Services/StoreManager.swift` (products → purchase → restore → entitlement mirror). Vecka Pro gates PDF/CSV export, removes the free-tier caps (3 custom events, 3 trips), and unlocks premium theme presets (Wagashi, Kincha, Vermilion, Sometsuke — gated via `JohoThemePreset.isPremium`; locked cards show a PRO pill and route to the paywall). Paywall: `Vecka/Views/PaywallView.swift`; entry points in Settings, Expense list export menu, Events and Trips add buttons, and locked theme cards.
